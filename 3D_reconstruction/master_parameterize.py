...
INPUT_DIR = Path(__file__).parent / "3D_data"
OUTPUT_DIR = Path(__file__).parent / "3D_data_params"
...
# Import additional functions from existing codebase if available
try:
    from D2_Trimdata import (
        find_gait_cycle_boundaries, calculate_duty_factors_proportion,
        calculate_step_frequencies_per_leg, calculate_leg_lengths,
        calculate_lateral_footfall_maximum,
        calculate_minimum_pull_off_force, calculate_minimum_pull_off_force_mass
    )
    HAS_D2_FUNCTIONS = True
except ImportError:
    HAS_D2_FUNCTIONS = False
    print("Warning: Could not import D2_Trimdata functions. Some advanced parameters may not be calculated.")
...
def calculate_all_parameters(data, duty_factor_df, branch_info, species_data, config, normalization_factor, dataset_name=None):
    ...
    for key in ['frame', 'time', 'speed', 'speed_normalized', 'slip_score',
                ...
                'head_distance_foot_8', 'head_distance_foot_14', 'total_foot_slip',
                ...
                'front_feet_avg_branch_distance', 'middle_feet_avg_branch_distance', 'hind_feet_avg_branch_distance',
                'all_feet_avg_branch_distance']:
        results[key] = []
    ...
    for frame in range(num_frames):
        ...
        results['immobile_foot_8_branch_distance'].append(immobile_foot_distances.get(8, np.nan))
        results['immobile_foot_9_branch_distance'].append(immobile_foot_distances.get(9, np.nan))
        results['immobile_foot_10_branch_distance'].append(immobile_foot_distances.get(10, np.nan))
        results['immobile_foot_14_branch_distance'].append(immobile_foot_distances.get(14, np.nan))
        results['immobile_foot_15_branch_distance'].append(immobile_foot_distances.get(15, np.nan))
        results['immobile_foot_16_branch_distance'].append(immobile_foot_distances.get(16, np.nan))
    ...
    # Calculate biomechanics parameters if functions are available
    biomechanics_summary = {}
    if HAS_D2_FUNCTIONS:
        try:
            # Calculate minimum pull-off force (average across frames)
            pull_off_forces = []
            foot_plane_distances = []
            cumulative_foot_spreads = []
            l_distances = {1: [], 2: [], 3: [], 4: [], 5: []}
            ...
            for frame in range(num_frames):
                ...
                force_value, intermediate = calculate_minimum_pull_off_force(
                    data, frame, branch_info, com_positions, dataset_name=None
                )
                if not np.isnan(force_value):
                    pull_off_forces.append(force_value)
                    if 'foot_plane_distance' in intermediate:
                        foot_plane_distances.append(intermediate['foot_plane_distance'])
                    if 'l_distances' in intermediate:
                        for i, l_dist in enumerate(intermediate['l_distances'][:5], 1):
                            if not np.isnan(l_dist):
                                l_distances[i].append(l_dist)
            
            # Store summary statistics
            if pull_off_forces:
                biomechanics_summary['Minimum_Pull_Off_Force'] = np.mean(pull_off_forces)
            else:
                biomechanics_summary['Minimum_Pull_Off_Force'] = np.nan
            
            if foot_plane_distances:
                biomechanics_summary['Foot_Plane_Distance_To_CoM'] = np.mean(foot_plane_distances)
            else:
                biomechanics_summary['Foot_Plane_Distance_To_CoM'] = np.nan
            
            # Cumulative foot spread (would need separate calculation)
            biomechanics_summary['Cumulative_Foot_Spread'] = np.nan  # TODO: Implement
            
            # L distances (average across frames)
            for i in range(1, 6):
                if l_distances[i]:
                    biomechanics_summary[f'L_Distance_{i}'] = np.mean(l_distances[i])
                else:
                    biomechanics_summary[f'L_Distance_{i}'] = np.nan
        except Exception as e:
            print(f"  ⚠ Could not calculate biomechanics parameters: {e}")
            # Set all to NaN
            biomechanics_summary = {
                'Minimum_Pull_Off_Force': np.nan,
                'Foot_Plane_Distance_To_CoM': np.nan,
...
def save_parameterized_data(dataset_name, data, results, duty_factor_df, branch_info, 
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        ...
        # Sheet 2: Speed (renamed from Kinematics, with stride/step length parameters)
        ...
        # Sheet 3: CoM (with branch distances for overall, head, thorax, gaster)
        com_df = pd.DataFrame({
            'Frame': results['frame'],
            'Time': results['time'],
            'CoM_X': results['com_x'],
        ...
        # Sheet 4: Duty Factor (per-frame attachment data + duty factor percentages)
        ...
        # Sheet 5: Behavioral (new sheet with Slip_Score, Gaster angles, Head distances, Total_Foot_Slip)
        ...
        # Sheet 9: Biomechanics (if calculations available)
        biomechanics_data = {}
        if 'biomechanics_summary' in results:
            biomechanics_df = pd.DataFrame({
                'Parameter': list(results['biomechanics_summary'].keys()),
                'Value': list(results['biomechanics_summary'].values())
            })
            biomechanics_df.to_excel(writer, sheet_name='Biomechanics', index=False)
        else:
            # Create empty Biomechanics sheet with expected parameters
            biomechanics_df = pd.DataFrame({
                'Parameter': ['Minimum_Pull_Off_Force', 'Foot_Plane_Distance_To_CoM', 
                            'Cumulative_Foot_Spread', 'L_Distance_1', 'L_Distance_2', 
                            'L_Distance_3', 'L_Distance_4', 'L_Distance_5'],
                'Value': [np.nan] * 8
            })
            biomechanics_df.to_excel(writer, sheet_name='Biomechanics', index=False)
